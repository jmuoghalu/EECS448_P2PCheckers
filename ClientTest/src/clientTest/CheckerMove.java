...
/**
 * CheckerMove stores an arbitrary game move that may or may not be valid. Each
 * CheckerMove is made up of a start and end square position.
 */
public class CheckerMove implements Serializable {
	...
	/**
	 * Initialize a new empty CheckerMove.
	 * 
	 * @param start
	 *            null.
	 * @param end
	 ...
	 * 
	 * @param start
	 *            The place where a piece was originally.
	 * @param end
	 *            The place where a piece will end up.
	 */
	...
	/**
	 * Is the move a double jump? That is does the piece move two spaces
	 * diagonally. (Note: does not check if there is anything between to make it
	 * a valid move; this is done in CheckerBoardState).
	 * 
	...
	/**
	 * Is the move a single jump? That is does the piece move one space
	 * diagonally. (Note: only checks for locations, no other checking is done.)
	 * 
	 * @see CheckerBoardState
	 * @return The answer to the above question.
	 */
	...
	public String toString() {
		return "From " + this.getStart().getX() + ", " + this.getStart().getY() + " to " + this.getEnd().getX() + ", "
				+ this.getEnd().getY();
	}
